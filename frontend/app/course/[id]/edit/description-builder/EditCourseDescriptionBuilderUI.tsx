...
type EditCourseDescriptionBuilderUIProps = {
  course: CourseEntity;
};

export default function EditCourseDescriptionBuilderUI({
  course,
}: EditCourseDescriptionBuilderUIProps) {
  const queryClient = useQueryClient();
  const [courseDescription, setCourseDescription] = useState<string>(
    course.description || DEFAULT_DESCRIPTION
  );

  const { mutate: updateCourseDescription, isPending } = useMutation({
    mutationFn: async () =>
